...
export class TfsecurityWrapper {
  ...
  run() {
    ...
    this.outputChannel.appendLine("Running tfsecurity to update results");

    if (!this.checkTfsecurityInstalled()) {
      return;
    }
    ...
    this.workingPath.forEach((workingPath) => {
      ...
      execution.on("exit", function (code) {
        if (code !== 0) {
          vscode.window.showErrorMessage("tfsecurity failed to run");
          outputChannel.show();
          return;
        }
        vscode.window.showInformationMessage(
          "tfsecurity ran successfully, updating results"
        );
        outputChannel.appendLine("Reloading the Findings Explorer content");
        setTimeout(() => {
          vscode.commands.executeCommand("tfsecurity.refresh");
        }, 250);
      });
    });
  }
  ...
  updateBinary() {
    ...
    const currentVersion = this.getInstalledTfsecurityVersion();
    if (currentVersion.includes("running a locally built version")) {
      this.outputChannel.appendLine(
        "You are using a locally built version which cannot be updated"
      );
    }

    if (semver.lt(currentVersion, "0.39.39")) {
      vscode.window.showInformationMessage(
        `Self updating was not introduced till v0.39.39 and you are running ${currentVersion}. Pleae update manually to at least v0.39.39`
      );
    }
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        "There was a problem with the update, check the output window"
      );
  ...
  showCurrentTfsecurityVersion() {
    const currentVersion = this.getInstalledTfsecurityVersion();
    if (currentVersion) {
      vscode.window.showInformationMessage(
        `Current tfsecurity version is ${currentVersion}`
      );
    }
  }
  ...
  private getBinaryPath() {
    ...
    var binary = config.get("binaryPath", "tfsecurity");
    if (binary === "") {
      binary = "tfsecurity";
    }
  ...
  private checkTfsecurityInstalled(): boolean {
    ...
    try {
      ...
      this.outputChannel.show();
      this.outputChannel.appendLine(
        `tfsecurity not found. Check the tfsecurity extension settings to ensure the path is correct. [${binaryPath}]`
      );
      return false;
    }
    return true;
  ...
  private getInstalledTfsecurityVersion(): string {
    if (!this.checkTfsecurityInstalled) {
      vscode.window.showErrorMessage(
        "tfsecurity could not be found, check Output window"
      );
      return "";
    }
