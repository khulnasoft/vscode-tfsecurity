...
import { TfsecurityTreeItem, TfsecurityTreeItemType } from "./explorer/tfsecurity_treeitem";
...
class FileIgnores {
  public constructor(
    ...
    public readonly ignores: IgnoreDetails[]
  ) {}
}

const tfsecurityIgnoreDecoration = vscode.window.createTextEditorDecorationType({
  fontStyle: "italic",
...
function triggerDecoration() {
  const config = vscode.workspace.getConfiguration("tfsecurity");
  ...
  timeout = setTimeout(updateTfsecurityIgnoreDecorators, 500);
}

function updateTfsecurityIgnoreDecorators() {
  if (!activeEditor) {
    return;
  }
  const regEx = /tfsecurity:ignore:([A-Z]+?\d{3})/g;
  const text = activeEditor.document.getText();
  const tfsecurityIgnores: vscode.DecorationOptions[] = [];
  ...
  while ((match = regEx.exec(text))) {
    ...
    const message = getTfsecurityDescription(match[1]);
    const decoration = {
    ...
    tfsecurityIgnores.push(decoration);
  }
  activeEditor.setDecorations(tfsecurityIgnoreDecoration, tfsecurityIgnores);
}

function getTfsecurityDescription(tfsecurityCode: string) {
  var check = CheckManager.getInstance().get(tfsecurityCode);
  if (check === undefined) {
    return "[Uknown tfsecurity code]";
  }
  return check;
}
...
function rerunIfRequired() {
  ...
  if (reRunOnIgnore) {
    setTimeout(() => {
      vscode.commands.executeCommand("tfsecurity.run");
    }, 1000);
  } else {
    vscode.window.showInformationMessage(
      "You should refresh the treeview after ignoring"
    );
  }
}
  ...
  await vscode.window.showTextDocument(vscode.Uri.file(filename)).then((e) => {
    e.edit((edit) => {
      for (let index = 0; index < ignores.length; index++) {
        ...
        const ignoreCode = `#tfsecurity:ignore:${element.code}`;
        ...
        if (element.startLine === element.endLine) {
          let errorLine = vscode.window.activeTextEditor?.document.lineAt(
            element.startLine
          );
          if (errorLine !== null && errorLine !== undefined) {
            let ignoreLinePos = element.startLine;
            ignoreLine =
              vscode.window.activeTextEditor?.document.lineAt(ignoreLinePos);
        ...
        if (
          ignoreLine !== undefined &&
          ignoreLine.text !== undefined &&
          ignoreLine.text.includes("tfsecurity:")
        ) {
          edit.insert(
            new vscode.Position(ignoreLine.lineNumber - 1, 0),
            `${ignoreCode}\n`
          );
        } else {
...
const ignoreAllInstances = async (
  element: TfsecurityTreeItem,
  issueProvider: TfsecurityIssueProvider,
  outputChannel: vscode.OutputChannel
) => {
  outputChannel.show();
  ...
  let severityIgnore = element.treeItemType === TfsecurityTreeItemType.issueSeverity;
  for (let index = 0; index < issueProvider.resultData.length; index++) {
    ...
    if (severityIgnore && r.severity !== element.severity) {
      continue;
    }
    if (!severityIgnore && r.code !== element.code) {
      continue;
    }
  ...
  await edits
    .reduce(
      (p, x) => p.then((_) => addIgnore(x.filename, x.ignores, outputChannel)),
      Promise.resolve()
    )
    .then(() => {
